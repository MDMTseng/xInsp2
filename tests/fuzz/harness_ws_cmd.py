"""r7 #1 — WebSocket cmd JSON parser fuzzer.
...
Writes `_results_ws_cmd.json` next to this file.
"""
...
from _common import BackendProc, open_ws, port_open, fuzz_iters  # noqa: E402

import os  # noqa: E402

ITERS = fuzz_iters(1200)
...
def main() -> int:
    ...
    findings: list[dict] = []
    print(f"[ws_cmd] iters={ITERS} seed={SEED}", flush=True)
    ...
    with BackendProc() as bp:
        ...
        recent_payloads: list[tuple[str, bytes]] = []
        ...
        for i in range(ITERS):
            ...
            # liveness probe periodically. The server is single-client, so
            # we MUST close the fuzz socket before opening the probe socket
            # — otherwise the probe's connect is rejected with HTTP 503
            # `single-client-busy` (that is the server enforcing its
            # contract, not a crash; it was the r7 P1 "accept-stall" wart).
            # We always reopen the fuzz socket afterwards to continue.
            probe_every = 50 if ITERS < 400 else 200
            if (i + 1) % probe_every == 0:
                ...
                if not ok:
                    ...
                    if not ok2:
                        ...
                        print("[STALL] recovered after wait", flush=True)
                if time.time() - last_status_t > 8.0:
                    print(f"[ws_cmd] iter={i+1}/{ITERS} alive ({info})", flush=True)
                    last_status_t = time.time()
                # Reopen the fuzz socket (closed before the probe) so the
                # next iteration can keep sending.
                try:
                    ws = open_ws(timeout=8.0)
                except Exception:
                    if not port_open():
                        findings.append({"iter": i, "kind": "reopen_failed_port_closed", "fatal": True})
                    break
        ...
        ok, info = liveness_check()
        print(f"[ws_cmd] done. final liveness: {ok} ({info}). findings={len(findings)}")
