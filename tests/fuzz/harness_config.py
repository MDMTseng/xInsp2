"""r7 #2 — project.json / instance.json / plugin.json fuzzer.

Targets the validation logic in `backend/include/xi/xi_plugin_manager.hpp`
and the project loader. We synthesize candidate project trees in temp
dirs and call `Client.open_project(dir)`.
...
We do NOT require any particular plugin to actually compile (we use a
no-DLL plugin manifest) — just that the loader's validation handles
...
from _common import BackendProc, WS_URL, fuzz_iters  # noqa: E402

from xinsp2 import Client, ProtocolError  # noqa: E402

ITERS = fuzz_iters(500)
...
def make_baseline(root: Path) -> dict:
    ...
    plugin DLL — the loader path we care about (manifest validation)
    runs before DLL load is attempted.
    """
...
def main() -> int:
    ...
    findings: list[dict] = []
    print(f"[config] iters={ITERS} seed={SEED}", flush=True)

    tmp_root = Path(tempfile.mkdtemp(prefix="xinsp2_fuzz_cfg_"))
    print(f"[config] tmp_root={tmp_root}")
    ...
    with BackendProc():
        ...
        try:
            for i in range(ITERS):
                ...
                err = None
                try:
                    c.open_project(str(proj_dir), timeout=20.0)
                ...
                if (i + 1) % 25 == 0 and time.time() - last_status_t > 8.0:
                    print(f"[config] iter={i+1}/{ITERS} pingable={pingable} last_dt={dt:.2f}s strategy={meta['strategy']}", flush=True)
                    last_status_t = time.time()
        finally:
            try:
                c.shutdown()
    ...
    print(f"[config] done. findings={len(findings)}")
