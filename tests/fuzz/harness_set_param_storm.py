"""r8 #5 — set_param storm during continuous mode.

Open multi_source_surge -> compile inspect.cpp -> start fps=60. From a
worker thread, blast set_param as fast as possible for DURATION_S; from
another thread drain events/binary. Look for crashes, dropped responses,
backend hang.
...
def main() -> int:
    findings: list[dict] = []
    print(f"[set_param_storm] duration={DURATION_S}s", flush=True)
    ...
    with BackendProc():
        ...
        try:
            ...
            print(f"[set_param_storm] param_names={names[:5]}...", flush=True)
            ...
            stats = {"sets": 0, "set_errs": 0, "exc": 0, "drained": 0,
                     "max_lat_ms": 0.0}

            def storm():
                seq = 0
                while not stop_flag.is_set():
                    nm = names[seq % max(len(names), 1)]
                    seq += 1
                    val = (seq * 7) % 1000
            ...
            while time.time() - t0 < DURATION_S:
                time.sleep(1.0)
                print(f"[set_param_storm] t={int(time.time()-t0)}s "
                      f"sets={stats['sets']} drained={stats['drained']} "
                      f"max_lat={stats['max_lat_ms']:.1f}", flush=True)
            ...
            sets_per_s = stats["sets"] / DURATION_S if DURATION_S else 0
