...
test('preview_sink: multi-group output, live binary push, content-hash dedup', async () => {
    await withBackend(async (c) => {
        ...
        // --- Live binary push: gradient + thumb (bright) + inverted (dark) = 3 frames ---
        ...
        const grad = frames.find(f => f.pg === 'bright' && f.key === 'gradient');
        assert.ok(grad, 'bright/gradient frame pushed');
        assert.deepEqual([grad.w, grad.h, grad.c], [320, 240, 1], 'frame header dims');
        assert.ok(grad.jpegLen > 0, 'frame carries a JPEG payload');
        assert.ok(frames.some(f => f.pg === 'dark' && f.key === 'inverted'), 'dark/inverted frame pushed');
        ...
        // (Dedup is now GLOBAL in the host cache — gradient & thumb share content so the
        ...
        assert.equal(bright.image_count, 2, 'bright has gradient + thumb');
        assert.ok(!('jpeg_b64' in bright), 'get (tab) carries no pixels');
        const bd = JSON.parse(bright.data);
        assert.equal(bd.gain, 7); assert.equal(typeof bd.score, 'number');
        assert.deepEqual(bd.$layout.map(e => [e.key, e.kind]),
            [['frame', 'value'], ['score', 'value'], ['gain', 'value'], ['gradient', 'image'], ['thumb', 'image']], 'layout order + kinds');
        ...
        const im = await exch({ command: 'get_image', pg: 'bright', key: 'gradient' });
        assert.equal(im.found, true, 'get_image found');
        assert.deepEqual([im.w, im.h, im.channels], [320, 240, 1], 'dims round-trip');
