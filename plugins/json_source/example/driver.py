"""json_source example — proof that the shipped demo project actually runs.

Opens plugins/json_source/example, compiles its script, ticks the source, and
asserts the script read the document off the pack plane and reached a verdict.
...
Run:  python plugins/json_source/example/driver.py
"""
...
def main() -> int:
    ...
    fails: list[str] = []
    proc = spawn_backend(PORT, ROOT / f"backend_{PORT}.log", tag="xi_json_source_ex")
    try:
        c = connect(PORT)
        assert c, "no connect"
        ...
        if not results:
            fails.append("no run_result — the source never ticked")
        else:
            # The shipped document is width_mm=12.7 <= pass_limit=15, so the
            # example's own verdict must be ok(1). A wrong code means the script
            # could not read the document (the thing this example demonstrates).
            codes = {code for code, _ in results}
            if 1 not in codes:
                fails.append(f"expected an ok(1) verdict from the shipped document, saw {sorted(codes)}")
