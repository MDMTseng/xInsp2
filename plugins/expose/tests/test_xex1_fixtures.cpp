...
int regen(const std::string& dir, const std::vector<Frame>& frames) {
    ...
    manifest.set("note",
        ...
        "by hand. Each *.bin is decoded and checked against this manifest by "
...
int verify(const std::string& dir, const std::vector<Frame>& frames) {
    ...
    for (const auto& f : frames) {
        ...
        std::printf("[xex1] ok %-10s %zu bytes\n", f.name.c_str(), got.size());
    }
    return failures;
}
...
int main(int argc, char** argv) {
    ...
    if (do_regen) return regen(dir, frames);

    int failures = verify(dir, frames);
    if (failures == 0) {
        std::printf("\nALL XEX1 FIXTURE GOLDENS MATCH (%zu frames)\n", frames.size());
        return 0;
    }
