"""data_output example — proof that the shipped demo project actually runs.

Opens plugins/data_output/example, compiles its script, runs it, and asserts the
script read the plugin's config surface and got an honest refusal from `save`.
...
Run:  python plugins/data_output/example/driver.py
"""
...
def main() -> int:
    ...
    fails: list[str] = []
    proc = spawn_backend(PORT, ROOT / f"backend_{PORT}.log", tag="xi_data_output_ex")
    try:
        c = connect(PORT)
        assert c, "no connect"
        ...
        if not results:
            ...
            # ok(1) means the config surface answered AND `save` refused with
            # not_implemented — the two claims this example makes.
            codes = {code for code, _ in results}
            if 1 not in codes:
                fails.append(f"expected ok(1) from the config surface, saw {sorted(codes)}")
