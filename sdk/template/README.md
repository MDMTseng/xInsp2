...
Output: `my_plugin.dll` next to `plugin.json`. The host expects this layout.
...
In VS Code: click `+` in **Instances** view → pick `my_plugin` → name it.
...
blocks. Builds to `my_plugin_test.exe`.

```bat
cmake --build build --config Release --target my_plugin_test
.\my_plugin_test.exe
```
...
That's the whole protocol. Look at `my_plugin.cpp` and `ui/index.html`
...
host runs baseline (8 tests) on first load, writes cert.json on pass
         ↓
host calls xi_plugin_create(host_api, name)            → new MyPlugin(...)
         ↓
...
`XI_PLUGIN_IMPL(MyPlugin)` generates all six C-ABI exports for you.
...
- **Cert is invalidated by DLL changes**: rebuild → next load re-runs
  ...
  `my_plugin_test.exe` locally to write cert proactively.
...
my_plugin/
├── plugin.json            ← manifest
├── my_plugin.cpp          ← all your C++ goes here (or split as you grow)
├── ui/index.html          ← webview UI
├── CMakeLists.txt         ← uses xInsp2's cmake helper
├── tests/
│   ├── test_native.cpp    ← XI_TEST blocks; compiles to my_plugin_test.exe
│   ├── test_ui.cjs        ← exports run(h); drives the live UI
│   └── screenshots/       ← created by h.shot(...)
├── my_plugin.dll          ← build output (placed beside plugin.json)
├── my_plugin_test.exe     ← native test runner
...
- **Debug prints**: `std::fprintf(stderr, "[my_plugin] %d\n", x)` shows
  up in the backend's output channel
- **Don't modify `xInsp2/`** — `git pull` on the host should never
  touch your work
- **Class name + file name**: scaffold emits `MyPlugin` (Pascal) +
  `my_plugin` (snake) + `MY_PLUGIN` (UPPER for macros) — keep them in
  sync if you rename
