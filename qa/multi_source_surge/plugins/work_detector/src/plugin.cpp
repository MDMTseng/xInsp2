...
public:
    ...
    void process(xi::PackIn& in, xi::PackOut& out) override {
        int sleep_ms = (int)in.i64_or("sleep_ms", default_sleep_ms_.load());
        if (sleep_ms < 0)   sleep_ms = 0;
        if (sleep_ms > 200) sleep_ms = 200;

        uint64_t seq     = 0;
        uint64_t src_tag = 0;
        auto img = in.image("img");
        if (img && img->pixels) {
            const uint8_t* d = static_cast<const uint8_t*>(img->pixels);
            std::memcpy(&seq,     d,     sizeof(seq));
        ...
        out.i64("seq",               (int)(seq & 0x7fffffff));
        out.i64("src_tag_lo",        (int)(src_tag & 0x7fffffff));
        out.i64("processed_total",   total);
        out.i64("processed_for_src", for_src);
    }
...
XI_PLUGIN_PACK_DOOR(WorkDetector)
