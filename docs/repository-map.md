...
| `sdk/` | Plugin/project SDK: `scaffold.mjs`, `create_plugin.sh`, `cmake/` module, `templates/`, `host_mock/`, `testing/` helpers, worked `qa/`. Lets you author a plugin without touching the host. | Yes (dev tooling shipped in the tree/zip) | unspecified (no own manifest) | Per-plugin: `cmake -S <plugin> -B <plugin>/build -A x64; cmake --build … --config Release` | `xi_test.hpp` helpers in `testing/`; scaffold self-tests | Plugin DLLs (built against the SDK cmake module) | Plugins compile against **ABI v11** |
