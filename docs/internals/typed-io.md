...
- **By key constant.** A plugin door reads/writes with its `_keys.gen.h`
  ...
  silent default. (Since the pack-v3 slab, a typed scalar read is a raw load —
  the container stores scalars raw and produces canonical msgpack only at the
  serialization edge; see [`pack-plane.md`](./pack-plane.md).)
