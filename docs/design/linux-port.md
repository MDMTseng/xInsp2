...
| `examples/fl_r7_fuzz/evil_worker.cpp` (FL r7) | **Win32-only by design.** This is a fuzz harness counterpart for the host-side IPC reader; it directly mimics what `xinsp-worker.exe` does on the wire (named-pipe client) so it inherits `xi_ipc.hpp`'s current Windows-only constraint. Gated in `backend/CMakeLists.txt` with `if(WIN32)` and tagged with `TODO(linux)` keyed off the eventual POSIX `Pipe` backend. The Python harness driver (`harness_evil_worker_host.py`) is portable, but won't have anything to drive on Linux until xi_ipc gets a POSIX backend. |
