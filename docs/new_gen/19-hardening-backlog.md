...
| H4 | **`qa_recipe_script_instance`** — after an instance reset, `factor` stays 42 instead of returning to default 5. Pre-existing behavioral bug (fails on master base too), the last real entry in the quarantine list. | `examples/qa_recipe_script_instance` + instance-reset path | Medium — reset-semantics correctness | M |
