...
| H3 | **QA drivers share default port 7917 → squatter cross-talk.** A stale/foreign backend on 7917 answers a different test's driver; the test's own backend logs "failed to bind" and the driver silently connects to the wrong process (symptom: `files=0`, false FAIL/PASS). Fix: sweep all QA drivers to `examples/lib/ports.free_port()`. | `examples/qa_*/driver.py` (found by qa_pack_param_modulation, qa_pack_feedback, qa_pack_walk) | Medium — test-infra false results under parallelism | S (sweep) |
| H4 | **`qa_recipe_script_instance`** — after an instance reset, `factor` stays 42 instead of returning to default 5. **DIAGNOSED — TEST-EXPECTATION BUG, not a backend defect (deferred: fix lives in driver.py, owned by the QA-driver-infra agent).** The driver's step 4 calls `compile_and_load` again and expects the script instance to revert to its source default. But `compile_and_load` is a HOT-RECOMPILE that DELIBERATELY replays the tuned instance defs cached in `g_eng.instance_def_cache` (`service_cmd_lifecycle.cpp:375-400`, exact sibling of the param replay) — precisely so a recompile does NOT wipe operator-tuned/taught/calibrated instances. `set_instance_def("scaler",{factor:42})` populates that cache (`service_cmd_project.cpp:176`); the reload re-applies it, so `factor` stays 42 BY DESIGN. A genuine reset that clears the cache is `unload_script`/`close_project` (`service_cmd_lifecycle.cpp:632/919/1035`), not `compile_and_load`. Evidence: driver prints `default=5 → tuned=42 → after reset=42`. Fix = point the driver's "reset" at a real reset (unload_script, or close+reopen) before load_project; leave quarantined until the driver is corrected (the ratchet would otherwise flag it). Backend byte-unchanged. | `examples/qa_recipe_script_instance/driver.py` (driver, not backend) | Medium — reset-semantics correctness | M |
| H5 | **`qa_local_auto`** — driver expects VAR-era auto-emitted run_results (gets 0); the script was ported to the expose API but the driver's expectation was not. Fix: port the driver to current auto-emit semantics. | `examples/qa_local_auto/driver.py` | Low — stale test driver | S |
