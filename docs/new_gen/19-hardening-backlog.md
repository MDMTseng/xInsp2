...
| ~~RT5~~ | ~~**`cmd_remove_instance_` had NO quiesce guard.**~~ **DONE.** The only DLL-affecting lifecycle op that didn't quiesce the dispatch pool — closes R1's ledger-mis-attribution UAF window on remove. Now wrapped like every sibling. | `service_cmd_project.cpp` | High | quiesce guard; `qa/qa_remove_under_load`. |
...
|---|---|---|
| V1 | **Core JPEG ENCODER eviction** — delete the in-core encoder (`xi_jpeg.hpp` `encode_jpeg`) + the BACKEND-target `XINSP2_HAS_TURBOJPEG`; `compress_sink` becomes capability-only. | **STAGED** (polaris2/v12-encoder-eviction): pre-v12 `compress_sink` delegates to `xi.jpeg.encode` (per-call re-check, reentrancy/quarantine/$fault → in-core fallback) exactly as the decode eviction (doc 06 §1). The double-cache worry is RESOLVED — the capability path defers dedup to imgcodec's identical content cache; the host memo cache now wraps the in-core fallback ONLY (one cache per served path). turbojpeg already MOVED into imgcodec (behind the same opt-in, doc 14). The remaining v12 work is only the DELETION of the in-core encoder + BACKEND turbojpeg — the v11→v12 break, done with V2 at the cut. |
