...
| Status | Rows | Count |
|---|---|---|
| **GREEN (pack-only, live-service QA evidence)** | A1–A9, B1, B2, C1, C2, C3, C4, D1, D2, G1 | **18** |
...
fixturing_demo / io_stress / graph_demo, rows B3/B4) — plus four composition
rows owing one example each (E1/E2/E3 collapse into the graph-level
record→replay example doc 10 already names; F1 into a pack-mode config-swap
drive). **The Gate P2 verdict rendered on this measurement is in doc 10:
ACHIEVED WITH NAMED RESIDUALS (U1 open; U3 since resolved).** U2
