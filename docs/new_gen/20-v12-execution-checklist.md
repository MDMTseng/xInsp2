...
- [ ] **Examples tree**: port all `inspect.cpp` off Record (~117 files use
...
- [ ] **In-tree plugins**: the 19 plugin files using `xi::Record` — confirm each
      ...
      dead Record half.)
- [ ] App-team plugins/scripts recompile against the v12 SDK on their side (their
      port is done; this is their recompile-and-revalidate).
- [ ] GATE after Stage A: full 8-stage gate green with Record still present but
      no in-tree consumer. This is the "safe to cut" checkpoint.

**Stage B — the deletions (the actual break, one commit)**
- [ ] **ABI v12**: `XI_ABI_VERSION 11 → 12`, `XI_ABI_MIN_COMPAT → 12`
...
- [ ] **Formalize the capability plane** as documented v12 ABI (`xi.cap` /
...
- [ ] **Delete Record + its machinery**: `xi_record.hpp`, `xi_doc_pool.hpp`,
      ...
      obsolete with it.
- [ ] **Delete core image codec**: the `read_image_file` host_api slot + core's
...
- [ ] Full 8-stage `tools/gate.py` green on the cut branch (docs/build/sdk/ctest/
...
- [ ] `test_abi_freeze` re-baselined to the v12 `xi_host_api` layout (the slot
