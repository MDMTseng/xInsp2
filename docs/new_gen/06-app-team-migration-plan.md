...
| 8 | **`host_api->read_image_file` evicted to the `xi.image.decode` capability** (core-codec eviction, doc 14 roster). At v12 the host slot **and** its built-in stb fallback are deleted — an `xi.image.decode` provider (`imgcodec`) is the only decode engine. Pre-v12 the slot already delegates to the capability when an `imgcodec` instance is present and silently falls back to built-in stb otherwise, byte-for-byte and fail-mode-identical (proven: `cap_imgcodec_host_test`), so the swap is a non-event you can rehearse TODAY | Any plugin of yours that calls `host_api->read_image_file` stops compiling at v12 | Ensure an `imgcodec` (or other `xi.image.decode` provider) instance is in the project; call the capability through the `xi.cap` funnel with a `bin "data"` request (§3.7). The built-in stb path in core is gone at the cut | **S** per calling plugin |
| 9 | **Preview JPEG ENCODER evicted to the `xi.jpeg.encode` capability** (core-codec eviction, doc 14 roster — the encode twin of row 8). At v12 the in-core encoder (`xi_jpeg.hpp` `encode_jpeg`) **and** the BACKEND-target `XINSP2_HAS_TURBOJPEG` are deleted — the `imgcodec` provider is the only encode engine, and it owns the SIMD (turbojpeg) path (doc 14). Pre-v12 the host `compress_sink` already delegates to the capability when an `imgcodec` instance is present and falls back to the in-core encoder otherwise, byte-identical engine-for-engine (proven: `cap_jpeg_encode_host_test`). Dedup defers to imgcodec's content cache on the capability path (no host double-cache); the host memo cache wraps the in-core fallback only. Purely a HOST convenience (`host_api->compress_image` / the `xi.preview@1` interface) — no app-team plugin calls it directly | Nothing YOU own: `compress_image` is a host service, not a plugin surface. Core loses `-DXINSP2_HAS_TURBOJPEG`; turbojpeg (if deployed) links into the `imgcodec` DLL instead | Ensure an `imgcodec` instance is present for preview-encode parity; nothing to port | **S** (host-side; zero per plugin) |
...
Live QA references you can crib from: `qa/qa_use_pack_door` (build →
...
`ScriptTypedPack` with a schema of key slots (see `qa/qa_pack_walk`).
...
- **E1 — capability plane not live in their deployment**: `xi.image.decode`
  ...
  **no `imgcodec` provider is registered**, so `available("xi.image.decode")==0`
  ...
  `qa/qa_cap_imgcodec/instances/codec/`); OR (b-ii) **enable machine-level
...
- **E2 — full-resolution preview without raw-pixel WS load**: they downsample
  ...
  **fail-OPEN to raw**, never to nothing: absent provider / `preview_compress`
  ...
  (`record_replay_pack_test` green). Proof: `qa/qa_jpeg_preview`
  ...
  (`ui/index.html`) and `qa/lib/xex1.py`.
