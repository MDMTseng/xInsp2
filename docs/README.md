...
| [`reference/ipc-shm.md`](./reference/ipc-shm.md) | SHM/process-isolation — **REMOVED 2026-05**; historical reference only |
...
| [`design/wire-format-msgpack.md`](./design/wire-format-msgpack.md) | Replace cJSON's JSON round-trip (the per-`process()` serialization tax) with MessagePack via CWPack end-to-end (internal ABI + WS), RAII refcounted buffers + image handles to minimize copy; bench-backed (cJSON 21× slower). Design — Phase 1 codec swap then Phase 2 msgpack-native Record. |
