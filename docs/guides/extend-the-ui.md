...
| `state_dropped` | A **warning notification** — the developer must see that hot-reload dropped their persisted `xi::state()` on a state-schema change, with the old→new schema versions. |
