"""FL r8 #3 — fire cmds while compile_and_load is in flight.

The WS handler thread is parked synchronously inside xi::script::compile
while a script compiles. We use a SECOND ws connection (which the
single-client design will refuse with HTTP 503) — but we ALSO send
cmds on the SAME socket from a separate Python thread, since the SDK
multiplexes on cid.

PASS criteria
-------------
- compile completes (success or ProtocolError, both ok).
- Concurrent ping/version cmds either:
  (a) get a rsp before compile finishes (means the dispatcher is async), OR
  (b) get a rsp after compile finishes (means cmds are queued FIFO and
      no one is lost or reordered), OR
  (c) raise ProtocolError, NOT timeout (a timeout = lost cmd).
- All cids have unique rsps; no rsp delivered to the wrong cid.
- No backend crash.
"""
...
ITERS = int(os.environ.get("FUZZ_ITERS", "10"))
...
def main() -> int:
    findings: list[dict] = []
    progress(f"cmd_during_compile started iters={ITERS}")
    ...
    with BackendProc():
        ...
        try:
            ...
            for i in range(ITERS):
                ...
                findings.append(rec)
                progress(f"cmd_during_compile iter {i+1}/{ITERS} "
                         f"compile_lat={int(compile_lat)}ms "
                         f"hammer_ok={hammer_oks}/{hammer_total}")
        finally:
            try: c.close()
...
def _write(findings) -> int:
    ...
    progress(f"cmd_during_compile done findings={len(findings)} fatal={fatals}")
