...
inspect sleeps a random 50-100ms. 4 workers/group absorb 20/s (cap ~53/s) so the
queues should NOT pile up. Groups are result_order:"arrival". The driver polls
...
def spawn(port):
    iso = Path(os.environ["LOCALAPPDATA"]) / "Temp" / "xi_gstress_iso"
