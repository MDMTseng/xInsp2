...
eviction — are pinned by the ctest unit plugins/cap_ui_egress_test.cpp (a timer-
driven pipeline cannot make those wall-clock-deterministic from out here).

Run:  python examples/qa_ui_egress/driver.py   (Windows; backend + plugins built)
"""
...
def main() -> int:
    ...
    print("VERDICT: PASS: xi.ui.egress rate-caps a fast source to the UI rate with a "
          "full-res jpeg preview, does ZERO encode work with no subscriber, and is a "
          "clean no-op (product plane byte-intact) when the cap is absent")
    return 0
