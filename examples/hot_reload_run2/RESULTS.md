...
> Note: this file is regenerated by `driver.py` on each run. The numbers
> below were captured before the VAR→`expose` migration (script then
> published `vars` events; it now publishes `expose` XEX1 frames on channel
> `main`). Acceptance logic and findings are unchanged — re-run to refresh.

## Outcome
- Total expose frames: 70
- Pre-reload frames: 38
...
- v2 version field appeared within 5 frames of reload: yes (offset=0)
- Largest gap in expose-frame stream: 8156 ms (straddles reload boundary: yes)
