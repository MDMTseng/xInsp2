...
public:
    ...
    std::string exchange(const std::string& cmd) override {
        ...
        if (command == "start") { start_(); return get_def(); }
        if (command == "stop")  { stop_();  return get_def(); }
        if (command == "fire") {                 // deterministic headless drive
        ...
        // P2-4: surface typo'd / unrecognised commands rather than the
        // legacy silent get_def() fallthrough.
