...
private:
    ...
    void emit_one_() {
        ...
        // v12 pack-plane emit: the frame rides under key "img"; seq/src_tag stay
        // stamped in the first 16 bytes (the script + detector read them there).
        xi::PackOut out = new_pack();
        if (!out.valid()) return;           // host has no pack plane
