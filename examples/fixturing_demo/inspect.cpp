...
void xi_inspect_entry(int) {
    ...
    // Surface the typed-I/O lineage + values through the `expose` plugin.
    xi::use("expose").process(xi::Record()
        .set("$channel", "fixturing")
        .set("loc_src",          loc_out.src())
        .set("n_poses",          located.orientation_count())
        .set("pose0_x",          pose0.x())
        .set("pose0_y",          pose0.y())
        .set("pose0_src",        pose0.src())
        .set("in_prov_current",  fit_in.prov_of("current"))
        .set("fit_na",           fit_out.is_na())
        .set("line_x1",          line.x1())   // baseline (10,0)-(10,40) shifted to pose0 (angle 0)
        .set("line_y2",          line.y2())
        .set("na_out_na",        na_out.is_na())
        .set("na_reason",        na_out.na_reason())
        .set("na_line_na",       na_line.is_na()));
}
