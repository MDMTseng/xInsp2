...
ask each which version it actually got.
  ...
  Phase 1 (same base name "dep.dll"):  expect a COLLISION — whoever loads first
    wins; the second plugin's LoadLibraryEx returns the already-loaded module, so
    it silently runs the wrong version (loaded path != the path it requested).

  Phase 2 (distinct names depA.dll / depB.dll):  expect NO collision — different
    base names are independent modules, each plugin gets its own version.
