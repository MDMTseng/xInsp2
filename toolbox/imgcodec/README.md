...
backend uses; wired in `toolbox/CMakeLists.txt`), else stb_image_write
...
it into any pipeline. Tests: `toolbox/build` ctest `cap_imgcodec_test` (dedup +
round-trip against the real DLL) and `qa/qa_cap_imgcodec` (live service).
