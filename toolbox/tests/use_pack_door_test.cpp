...
//      qa/qa_pack_feedback drives live.
...
// registry lookup → adapter → U3 sink rejection (-5) → door probe →
// run_pack_door — the real funnel's combined order. Same return-code
...
static int use_pack_process(const char* name, xi_pack_handle in, xi_pack_handle* out) {
    ...
    // Same slot as the real funnel: after adapter resolution, before the
    // quarantine/door gates.
    if (a->is_sink()) return -5;
    if (!a->has_pack_door()) return -4;
    try {
        ++g_door_calls;
