//
// test_frame_door.cpp — the host side of the xi.frame@1 carved data-plane door
// (polaris2 wave-2, docs/new_gen/08 Wave 2, steps 1-2).
//
// Covers:
//   1. Door probe — host->get_interface("xi.frame", 1) resolves the data plane
...
//   3. Refcount lifecycle — retain/release, sealed-frame immutability, and the
//      pooled-handle balance verified against ImagePool's own live count (a
//      frame's image entry mints exactly one pool handle, freed on last release).
//   4. Dispatch DUAL-CARRY — TriggerBus::emit_frame carries a Frame on the SAME
//      bus that TriggerBus::emit carries a Record, and neither bleeds into the
//      other (a frame event has frame!=NULL/images empty; a record event has
//      images!=empty/frame==NULL). Frame + image refs balance to baseline.
//
// The plugin-side door (blob_analysis's frame-in/frame-out) + the real end-to-end
// mock_camera->blob_analysis flow are exercised in the PLUGIN test
// (plugins/.../frame_pilot_test) against the actually-built DLLs.
//
#include <xi/xi_frame_abi.hpp>    // FrameRegistry, frame_v1_iface, install_frame_abi
...
static void test_door_probe() {
    SECTION("get_interface(\"xi.frame\", 1) resolves the data plane; wrong version NULL");
    xi::install_frame_abi();
    xi_host_api host = xi::ImagePool::make_host_api();
    CHECK(host.get_interface != nullptr);

    const void* v1 = host.get_interface("xi.frame", 1);
    CHECK(v1 != nullptr);
    CHECK(v1 == xi::frame_v1_iface());               // the process-stable singleton
    CHECK(host.get_interface("xi.frame", 2) == nullptr);  // only @1 published
    CHECK(host.get_interface("xi.frame", 0) == nullptr);
    CHECK(host.get_interface("xi.frameX", 1) == nullptr);

    // A frame-capable plugin caches this pointer once (Plugin::frame_iface()).
    const auto* fi = static_cast<const xi_frame_v1*>(v1);
    CHECK(fi->builder_new && fi->builder_seal && fi->get_i64 && fi->emit_frame);
}
...
static void test_build_read_roundtrip() {
    ...
    xi_frame_handle f = fi->builder_seal(b);
    CHECK(f != XI_FRAME_NULL);
    ...
    CHECK(fi->tag_of(f, "threshold") == XI_FRAME_TAG_I64);
    CHECK(fi->tag_of(f, "gray") == XI_FRAME_TAG_IMAGE);
    ...
    CHECK(fi->tag_at(f, 0) == XI_FRAME_TAG_I64);
    CHECK(fi->tag_at(f, 4) == XI_FRAME_TAG_IMAGE);
    CHECK(fi->key_at(f, 99, &klen) == nullptr && klen == 0);   // OOB
    CHECK(fi->tag_at(f, 99) == -1);

    // Release: the frame drops, its pool handle frees, pool balances to baseline.
    fi->release(f);
...
// ---------------------------------------------------------------------------
// (3) Refcount lifecycle: retain holds the frame alive past one release.
// ---------------------------------------------------------------------------
static void test_refcount_lifecycle() {
    SECTION("retain/release refcount + sealed-frame liveness");
    xi::install_frame_abi();
    const xi_frame_v1* fi = xi::frame_v1_iface();
    size_t base_frames = xi::FrameRegistry::instance().live_frames();

    xi_frame_builder b = fi->builder_new();
    fi->builder_add_i64(b, "n", 7);
    xi_frame_handle f = fi->builder_seal(b);                 // rc = 1
    CHECK(xi::FrameRegistry::instance().live_frames() == base_frames + 1);
    ...
    CHECK(xi::FrameRegistry::instance().live_frames() == base_frames);
    CHECK(fi->get_i64(f, "n", &v) == 0);

    // Abandon an unsealed builder — no leak, nothing in the frame table.
    xi_frame_builder b2 = fi->builder_new();
...
// ---------------------------------------------------------------------------
// (4) Dispatch dual-carry: Frame and Record on the SAME bus, no crosstalk.
// ---------------------------------------------------------------------------
static void test_dispatch_dual_carry() {
    SECTION("TriggerBus carries a Frame (emit_frame) alongside a Record (emit)");
    xi::install_frame_abi();
    xi_host_api host = xi::ImagePool::make_host_api();
    ...
    // --- FRAME currency: build a frame with an image + a scalar, emit it. ---
    std::vector<uint8_t> gray(16, 0); gray[0] = 255;
    xi_frame_builder b = fi->builder_new();
    fi->builder_add_i64(b, "seq", 11);
    fi->builder_add_image(b, "frame", 4, 4, 1, gray.data());
    xi_frame_handle f = fi->builder_seal(b);        // emitter's ref (rc 1)
    fi->emit_frame("cam_frame", XI_TRIGGER_NULL, f, 0);  // event takes a 2nd ref
    ...
    // The frame event: frame set, images empty; the frame reads back correctly.
    const xi::TriggerEvent* fe = nullptr;
    const xi::TriggerEvent* re = nullptr;
    for (auto& ev : got) {
        if (ev.frame != XI_FRAME_NULL) fe = &ev; else re = &ev;
    }
    CHECK(fe != nullptr);
    CHECK(re != nullptr);
    if (fe) {
        CHECK(fe->leader_source == "cam_frame");
        CHECK(fe->images.empty());                   // a frame event carries no image map
        int64_t seq = 0;
        CHECK(fi->get_i64(fe->frame, "seq", &seq) == 1 && seq == 11);
        xi_frame_image iv{};
        CHECK(fi->get_image(fe->frame, "frame", &iv) == 1 && iv.width == 4);
    }
    // The record event: images set, frame NULL (the Record path is untouched).
    if (re) {
        CHECK(re->leader_source == "cam_record");
        CHECK(re->frame == XI_FRAME_NULL);           // no frame bled into the Record path
    ...
    CHECK(pool_live() == base);   // every frame + image ref accounted for
}

int main() {
    std::printf("[test] xi.frame@1 carved data-plane door + dispatch dual-carry\n");
    test_door_probe();
