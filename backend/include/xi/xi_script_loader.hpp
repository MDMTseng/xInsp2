...
inline bool load_script(const std::string& dll_path, LoadedScript& out, std::string& err) {
    ...
    out.handle = h;
    out.path   = dll_path;
    // Contract: every pointer below is OPTIONAL. Missing symbols stay null;
    // callers MUST null-check before invoking. The reset / param / instance /
    // use / trigger / kv thunks are all auto-generated by
    // xi_script_support.hpp — only present if the user's script actually uses
    // the corresponding header. Load fails only if BOTH entry symbols
    // (xi_inspect_entry AND xi_inspect_entry_tv) are absent — see the check
    // after the GetProcAddress block; either one alone is sufficient.
